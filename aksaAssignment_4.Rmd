...
Let's start by loading the required packages and the original data set. It contains information about 21 Pharma ceuticsical companies.
...
#Reading the dataset
library(readr)
Pharmaceutics <- read.csv("./Pharmaceuticals.csv")
view(Pharmaceutics)
head(Pharmaceutics)
str(Pharmaceutics)
summary(Pharmaceutics)
dim(Pharmaceutics)
colMeans(is.na(Pharmaceutics))

row.names(Pharmaceutics) <- Pharmaceutics[,2]
Pharmaceutics <- Pharmaceutics[,-2]
```
...
Pharmaceutics.Que1 <- Pharmaceutics[,-c(1,11:13)]
```
...
Pharmaceutics.Que3 <-  Pharmaceutics %>% select(c(11,12,13)) %>% 
...
Thus, we can compile all of the data from the dataset and identify 3 distinct groups from the list of 21 Pharmaceuticsical companies.
